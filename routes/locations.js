...
router.put("/locations/:id", hasRole(["superAdmin", "admin", "organizador"]),schemaLocationPut, async (req, res) => {
    ...
    try {
        if (location) {
            ...
            res.json({ message: "O local foi editado." });
...
router.delete("/locations/:id", hasRole(["superAdmin", "admin", "organizador"]), async (req, res) => {
    ...
    try {
        if (location) {
            location.destroy()
            res.status(200).json({ message: "O local foi removido." });
        } else {
            res.status(404).json({ message: "Local com o ID informado não foi encontrado " });
        }
    } catch (error) {
        res.status(500).json({ message: "Um erro aconteceu" })
    }
