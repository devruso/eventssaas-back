    ...
    try {
        ...
        if (speaker) {
            if (event) {
                ...
                if(eventspeaker){
                    res.status(400).json({message:"Palestrante já cadastrado"})
                }else{
...
router.get("/eventspeakers/checkin/:speakerId/:eventId", async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Ocorreu um erro ao realizar o check-in." });
    }
});
...
router.delete("/eventspeakers/:id", hasRole(["superAdmin", "admin", "organizador"]), async (req, res) => {
    ...
    try {
        if (eventSpeaker) {
            ...
            res.json({ message: "qrcode deletado com sucesso." })
        }
    } catch (error) {
        res.status(500).json({ message: "Um erro aconteceu." })
    }
