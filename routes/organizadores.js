...
const { schemaUser, schemaUserPut, schemaProfilePut, schemaBioPut } = require("../utils/validate/schemas");

const router = Router();

router.get("/organizadores", hasRole(["superAdmin", "admin", "organizador"]), async (req, res) => {
    const { id } = req.query;
...
router.post("/organizadores", hasRole(["superAdmin", "admin"]),schemaUser, async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({ message: "Organizador criado com sucesso!", newUser });
    } catch (error) {
        return res.status(500).json({ message: 'Um erro ocorreu.' });
    }
});

router.put("/organizadores/:id", hasRole(["superAdmin", "admin", "organizador"]),schemaUserPut, async (req, res) => {
...
router.put("/organizadores/:id/profile", hasRole(["superAdmin", "admin", "organizador"]),schemaProfilePut  , async (req, res) => {
    const { id } = req.params;
...
router.put("/organizadores/:id/bio", hasRole(["superAdmin", "admin", "organizador"]),schemaBioPut, async (req, res) => {
    const { id } = req.params;
...
router.delete("/organizadores/:id", hasRole(["superAdmin", "admin"]), async (req, res) => {
    const { id } = req.params;
