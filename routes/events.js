...
router.put("/events/:id", hasRole(["superAdmin", "admin", "organizador"]), schemaEventPut, async (req, res) => {
    ...
    try {
        ...
        if (event) {
            ...
            res.json({ message: "O evento foi editado.", event });
...
router.delete("/events/:id", hasRole(["superAdmin", "admin", "organizador"]), async (req, res) => {
    ...
    try {
        if (event) {
            event.destroy()
            res.status(200).json({ message: "O evento foi removido" })
        } else {
            res.status(404).json({ message: "Evento com o ID informado não foi encontrado " });
